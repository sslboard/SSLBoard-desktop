...
- [ ] 1.1 Create orchestration hook `src/hooks/useManagedIssuanceFlow.ts`
...
- [ ] 1.2 Split UI sections into components under `src/components/issue/`
...
- [ ] 1.3 Update `src/pages/Issue.tsx` to compose the sections
...
- [ ] 2.1 Extract issuer validation to `src/lib/issuers/validation.ts`
  - Move `validateIssuerForm` logic and error strings
  - Keep validation pure and unit-testable (even if tests are not added yet)
- [ ] 2.2 Split UI into components under `src/components/settings/issuers/`
...
- [ ] 2.3 Replace `IssuerManager` with a small composition component
...
- [ ] 5.1 Standardize naming + handler conventions across touched modules
  - Prop handlers: `onX`
  - Local handlers: `handleX`
  - Boolean state: `isX` / `hasX`
- [ ] 5.2 Standardize import grouping across touched modules
