...
export function useCsrIssuanceFlow(
  ...
  csrPath: string | null,
  csrSource: CsrSource,
  managedKeyRef?: string | null,
) {
  const [startResult, setStartResult] = useState<StartCsrIssuanceResponse | null>(null);
  ...
  async function finalizeIssuance(result: StartCsrIssuanceResponse, token: number) {
    setFinalizing(true);
    setFinalizeFailed(false);
    setError(null);
    try {
      const record = await completeCsrIssuance({
  ...
  async function handleStart() {
    ...
    try {
      ...
      if (!csrPath) {
        throw new Error("Select a CSR file before starting issuance.");
      }
      const result = await startCsrIssuance({
        issuer_id: selectedIssuerId,
        csr_path: csrPath,
        csr_source: csrSource,
        managed_key_ref: managedKeyRef ?? undefined,
      });
      if (isStale(token)) {
        return;
      }
