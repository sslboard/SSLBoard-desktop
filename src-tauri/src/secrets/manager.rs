...
impl SecretManager {
    pub fn initialize(app: tauri::AppHandle) -> Result<Self> {
        ...
        let store = Arc::new(KeyringSecretStore::new("sslboard-desktop"));
    ...
    pub fn create_secret(
        ...
        self.store_secret(&id, &secret_value)?;
        ...
        if let Err(err) = self.metadata.insert(&record) {
            ...
            return Err(SecretError::Metadata(err.to_string()));
        }
    ...
    fn store_secret(&self, id: &str, secret_value: &str) -> Result<(), SecretError> {
        self.store
            .store(id, secret_value.as_bytes())
            .map_err(|err| match &err {
                SecretStoreError::Unavailable(_) => SecretError::Unavailable(err.to_string()),
                SecretStoreError::NotFound(_) => SecretError::NotFound(id.to_string()),
                SecretStoreError::Store(_) => SecretError::Store(err.to_string()),
            })
    }
